...
const UserTable = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("http://localhost:4000/salaryRoutes");
  ...
  const deleteUser = async (id) => {
    try {
      await axios.delete(`http://localhost:4000/salaryRoutes/${id}`);
    ...
    <div style={{ display: "flex" }}>
      <div style={{ flex: 1 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "20px" }}>
          <div>
            <Link to="/add-salary-details" style={{ textDecoration: "none" }}>
              <Button 
                variant="contained" 
                color="primary" 
                endIcon={<PersonAdd />}
          ...
          <div>
            ...
            <Button 
              ...
              style={{ marginRight: "30px", padding: "5px", backgroundColor: "#f0f0f0", color: "#934ae2"}} 
            >
              Generate Report
            </Button>
          </div>
            ...
            <TableBody>
              {filteredUsers.map((user, index) => (
                <TableRow key={index}>
                  ...
                  <TableCell>{user.basicSalary}</TableCell>
                  <TableCell>{user.otHours}</TableCell>
                  <TableCell>{user.otRatePerHour}</TableCell>
                  <TableCell>{user.allowances}</TableCell>
                  <TableCell>{user.totalSalary}</TableCell>
                  <TableCell>
                    {user._id && (
                      <Link to={{ pathname: `/update/${user._id}` }}>
